...
class AdaptiveCruiseControlEnv(gym.Env):
    ...
    def __observation_space(self):
        #[ego_speed, target_speed, distance, relative_speed] distance in m and speed in m/s
        return spaces.Box(low=np.array([0.0, 25.0, 0.0, -100.0], dtype=np.float32), high=np.array([100.0, 40.0, 100, 100.0], dtype=np.float32), shape=(4,), dtype=np.float32)
    ...
    def spawn_vehicle(self):
        #Spawn ego vehicle
        random_offset = rnd.randint(500, 550) # 70, 100
        ...
        random_leader_velocity = physics.kmh_to_ms(rnd.randint(50, 70)) # In m/s
        random_target_speed = rnd.randint(70, 130) # In km/h
        ...
        self.set_target_speed(physics.kmh_to_ms(random_target_speed))
    ...
    def __get_observation(self):
        ...
        return np.array([ego_speed, target_speed, distance, relative_speed], dtype=np.float32)
    
    def __compute_reward(self, observation):
        current_speed, desired_speed, distance, relative_speed = observation
        absolute_speed = relative_speed + current_speed
        ...
        isEgoAdaptSpeed = absolute_speed - speed_tolerance <= current_speed <= absolute_speed + speed_tolerance
        ...
        if not isObjectDetected:
            ...
            #Penalty for not adapting speed
            # if not isEgoAdaptSpeed:
            #     reward -= abs(absolute_speed - current_speed) / max(absolute_speed, 0.1)
            # else:
            #     reward += 10.0 - abs(absolute_speed - current_speed)
            reward = 0
    ...
    def __compute_done(self, observation):
        current_speed, _, distance, _  = observation
    ...
    def step(self, action):
        ...
        print(f"Reward: {reward}, Ego_speed: {physics.ms_to_kmh(observation[0])} km/h, Target_speed: {physics.ms_to_kmh(observation[1])} km/h, Leader speed: {physics.ms_to_kmh(self.leader_vehicle.get_velocity().length())} km/h, Distance: {observation[2]} m, Detected: {observation[2] < self.RADAR_RANGE}, Relative Speed: {physics.ms_to_kmh(observation[3])} km/h, Absolute Speed: {physics.ms_to_kmh(observation[0] + observation[3])} km/h")
